...
' تعريف كائن نظام الملفات
Dim fso
Set fso = Server.CreateObject("Scripting.FileSystemObject")

' تعريف الجذر والدليل الافتراضي
Dim rootPath, defaultDir
rootPath = "C:\"
defaultDir = "C:\wamp64\www\Inquiries"

' استرجاع المسار الحالي من GET أو استخدام الافتراضي
...
' التأكد من أن المسار يبدأ بالـ rootPath
If Left(current_dir, Len(rootPath)) <> rootPath Then
    current_dir = rootPath
End If

' حساب رابط الرجوع للمجلد الأب
...
If qs_download <> "" Then
    ...
    ' استخدام ADODB.Stream لقراءة الملف بنظام باينري
...
If qs_delete <> "" Then
    ...
    ' في حالة تقديم النموذج (POST) للتأكيد أو الإلغاء
...
If qs_show <> "" Then
    ...
    ' في حالة ملفات ASP يمكن استخدام Server.Execute (يجب أن يكون الملف ضمن التطبيق)
    Dim ext
    ...
    If ext = "asp" Then
        ...
        Response.End
    Else
        ' خلاف ذلك قراءة محتوى الملف وعرضه
        Dim fileContent, ts
        Set ts = fileObj.OpenAsTextStream(1, -2) ' 1 = ForReading، -2 = الترميز الافتراضي للنظام
...
If qs_edit <> "" Then
    ...
    If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
        If Request.Form("save") <> "" Then
            ...
            Dim tsWrite
            On Error Resume Next
            Set tsWrite = fileObj.OpenAsTextStream(2, -2) ' 2 = ForWriting
            If Err.Number <> 0 Then
    ...
    ' قراءة محتوى الملف للعرض في منطقة التحرير
...
' =======================
' عرض محتويات المجلد (Directory Listing)
' =======================
Dim folder, listItems(), count, subItem, items
On Error Resume Next
...
' تجهيز مصفوفة تجمع المجلدات والملفات
count = 0
